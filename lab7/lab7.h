...
class BST
{
	...
	Node* removeNode(Node *node, const T &data){
		...
		if(data < node->data_){
			node->left_ = removeNode(node->left_, data);
		}else if(data > node->data_){
			node->right_ = removeNode(node->right_, data);
		}else {
			...
			Node* small = node->right_;
			while(small && small->left_ != nullptr){
				small = small->left_;
			}
			node->data_ = small->data_;
			node->right_ = removeNode(node->right_, small->data_);
		}
		return node;
	}

	Node* copyNode(Node* rNode){
		if (rNode != nullptr)
		{
			Node *lNode = new Node(rNode->data_);
			lNode->left_ = copyNode(rNode->left_);
			lNode->right_ = copyNode(rNode->right_);
			return lNode;
		}
		return nullptr;
	}

	int depthSearch(const Node *node, const T &data){
		int depth;
		...
		depth = -1;
		if ((node->data_ == data) || (depth = depthSearch(node->left_, data)) >= 0 || (depth = depthSearch(node->right_, data)) >= 0){
			return ++depth;
		}
		return depth;
	}
...
public: 
	...
	{
		return depthSearch(this->root_, data);
	}
